...
public record GeneralSchema(WeaponSlot slot, int mastery, int maxRank, ModLayout layout, Optional<List<TriggerType>> triggerTypes) {
    public static final Codec<GeneralSchema> CODEC = RecordCodecBuilder.create(
            instance -> instance.group(
                    ...
                    TriggerType.CODEC.listOf().optionalFieldOf("triggerTypes").forGetter(GeneralSchema::triggerTypes)
    ...
    public static final StreamCodec<FriendlyByteBuf, GeneralSchema> STREAM_CODEC = StreamCodec.composite(
            ...
            ByteBufCodecs.optional(NeoForgeStreamCodecs.enumCodec(TriggerType.class).apply(ByteBufCodecs.list())), GeneralSchema::triggerTypes,
            GeneralSchema::new
    );
}
