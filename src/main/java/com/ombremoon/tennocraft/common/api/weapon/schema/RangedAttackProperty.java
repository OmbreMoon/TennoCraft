...
public class RangedAttackProperty extends AttackProperty {
    public static final Codec<RangedAttackProperty> CODEC = RecordCodecBuilder.create(
            ...
            ).apply(instance, RangedAttackProperty::new)
    );
    public static final StreamCodec<RegistryFriendlyByteBuf, RangedAttackProperty> STREAM_CODEC = StreamCodec.of(
            RangedAttackProperty::toNetwork, RangedAttackProperty::fromNetwork
    );
    private final RangedAttack attack;

    public RangedAttackProperty(RangedAttack attack, List<DamageValue> damageValues, float critChance, float critMultiplier, float status, NoiseLevel noise) {
        super(damageValues, critChance, critMultiplier, status, noise);
        this.attack = attack;
    }

    public RangedAttackProperty(RangedAttack attack, float critChance, float critMultiplier, float status, NoiseLevel noise, DamageValue... values) {
    ...
    private static RangedAttackProperty fromNetwork(RegistryFriendlyByteBuf buffer) {
        ...
        return new RangedAttackProperty(attack, damage, critChance, critMultiplier, status, noise);
    }

    private static void toNetwork(RegistryFriendlyByteBuf buffer, RangedAttackProperty schema) {
